...
#
# (c) 2016 Jon Cowie, Etsy Inc, Benedikt Bock
#
...
sub main {
  # Getopts:
  # j: Job name
  # l: Jenkins URL (including protocol e.g. http://)
  # u: User (optional)
  # p: password (optional)
  # f: Failure threshold
...
sub usage {
    print << "EOF";
usage: $0 -j <job> -l <url> -w <threshold> -c <threshold> -f <number> -b <threshold> [-u username -p password]
    ...
    Required arguments
        ...
        -l <url>        : Jenkins URL
                          Include the Protocol (e.g. http:// don't use a trailing /)

        -w <threshold>  : Warning Threshold (minutes)
                          WARNING when the last successful run was over <threshold> minutes ago.
                          0 to disbale

        -c <threshold>  : Critical Threshold (minutes)
                          CRITICAL when the last successful run was over <threshold> minutes ago.
                          0 to disable

        -f <number>     : CRITICAL when the last <threshold> runs was not successful, even if the last
                          successful run is within the -w and -c thresholds.
                          0 to disable

        -b <threshold>  : WARNING when last build took longer than <threshold> milliseconds.
                          0 to disable
